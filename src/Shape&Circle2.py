...
def isrect(blob):
    if len(img.find_rects(threshold=18000))!=0:
        return 1
    else:
        return 0
...
def istriangle(blob):
    ...
    if len(img.find_rects(threshold=18000))!=0:
        return 0
    elif len(img.find_circles(threshold=5000))!=0:
        return 0
    else:
        return 1

def ShapeRecognition2(Detect_Flag, temp_blob):
    temp_shape=0
    if temp_blob:
        temp_color = ColorRecognition(temp_blob)
        if Target_Shape==1 and isrect(temp_blob)==1:
            temp_shape = 1
            print("检测为长方形  ")
            img.draw_rectangle(temp_blob.rect())
            Pack_Shape = UserShapeDataPack(1, temp_color, temp_shape, temp_blob.cx()-160, 120-temp_blob.cy())
            Uart.write(Pack_Shape)
        if Target_Shape==2 and istriangle(temp_blob)==1:
            temp_shape = 2
            print("检测为三角形  ")#,end=''
            img.draw_cross(temp_blob.cx(), temp_blob.cy())
            Pack_Shape = UserShapeDataPack(1, temp_color, temp_shape, temp_blob.cx()-160, 120-temp_blob.cy())
            Uart.write(Pack_Shape)
        if Target_Shape==3 and iscircle(temp_blob)==1:
            temp_shape = 3
            print("检测为圆形  ")#,end=''
...
while (True):
    ...
    if Shape_State == 1 or Detect_State == 1:
        ...
        blob=FindMax(blobs)
        ShapeRecognition2(Detect_State,blob)
